...
class Sekolah extends REST_Controller
{
	...
	public function index_get()
	{
		...
		if ($id === NULL) {
			...
			if ($sekolah) {
				...
				$this->response($sekolah, REST_Controller::HTTP_OK); // OK (200) being the HTTP response code
			} else {
				// Set the response and exit
				$this->response([
					'status' => FALSE,
		...
		// Find and return a single record for a particular user.
			...
			$this->db->where(array("id_sekolah" => $id));
			$sekolah = $this->db->get("sekolah")->row_array();

			$this->response($sekolah, REST_Controller::HTTP_OK);
		}
	}
	...
	public function index_put()
	{
		...
		$this->db->where('id_sekolah', $this->put('id_sekolah'));
